...
Series configs define a planned sequence of newsletters that build on each other. The agent (Claude Code) reads these files to know which issue to run next, what topic and search queries to use, and which context file to pass to the synthesis step.
...
When you say "Run the next issue from `series/my-series.json`", the agent:

1. Reads the series JSON and finds the first issue with `"status": "pending"`
...
6. After sending, updates the issue: `status → "sent"`, fills `subject_line_chosen` and `sent_date`
...
```json
{
  ...
  "issues": [
    {
      "issue_number": 0,
      "title": "Short descriptive title for this issue",
...
| `issue_number` | integer | 0-indexed. Issue 0 is typically the overview/intro |
| `title` | string | Short label used by the agent to identify issues |
...
| `status` | string | `pending` → `sent` after the issue is sent |
  ...
  "issues": [
    {
      ...
      "topic": "Full topic phrase for the intro issue",
      "search_queries": [
    ...
    },
    {
      ...
      "topic": "Full topic phrase for issue 1",
      "search_queries": [
        "specific search query",
        "broader fallback query"
      ],
      "context_file": "context/your-series-issue-1.md",
      "variant": "long-form",
      "status": "pending",
      "subject_line_chosen": null,
      "sent_date": null
